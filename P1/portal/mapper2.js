...
class ProjectionMapper {
  constructor(p5ctxOrOptions, options = {}) {
    ...
    this.debugRawPreview = false; // keep raw preview opt-in so shortcuts stay mapped
  ...
  render() {
    ...
    if (this.debugBypass && this.debugRawPreview) {
      ...
      let y = -H2,
        x = -W2;
      noStroke();
      const columns = Math.ceil(Math.sqrt(this.surfaces.length));
    ...
    // For each surface: reuse cached inverse homography unless corners or DPR changed.
    shader(this.shader);
    for (let i = 0; i < this.surfaces.length; i++) {
      const s = this.surfaces[i];
      const cache = this._getRenderCache(s, dpr);
      if (!cache) continue; // skip degenerate
  ...
  _drawSurfaceBounds(bounds) {
    ...
    const W2 = width * 0.5;
    const H2 = height * 0.5;
    const x0 = bounds.x0 - W2;
    const y0 = bounds.y0 - H2;
    const x1 = bounds.x1 - W2;
    const y1 = bounds.y1 - H2;
    beginShape(TRIANGLE_STRIP);
    vertex(x0, y0, 0);
    vertex(x1, y0, 0);
    vertex(x0, y1, 0);
  ...
  _getRenderCache(surface, dpr) {
    ...
    const bounds = this._computeSurfaceBounds(x0, y0, x1, y1, x2, y2, x3, y3);
    if (!bounds) {
      surface.renderCache = {
        valid: false,
        dpr,
    ...
    const H = this._computeHomographyDLTFromCoords(
      x0 * dpr,
      y0 * dpr,
      x1 * dpr,
      y1 * dpr,
  ...
  _computeSurfaceBounds(x0, y0, x1, y1, x2, y2, x3, y3) {
    const pad = 1;
    const minX = Math.max(0, Math.floor(Math.min(x0, x1, x2, x3) - pad));
    const minY = Math.max(0, Math.floor(Math.min(y0, y1, y2, y3) - pad));
    const maxX = Math.min(width, Math.ceil(Math.max(x0, x1, x2, x3) + pad));
    const maxY = Math.min(height, Math.ceil(Math.max(y0, y1, y2, y3) + pad));
  ...
  _computeHomographyDLT(corners) {
    ...
    const tl = corners[0],
      ...
      br = corners[2],
      bl = corners[3];
    return this._computeHomographyDLTFromCoords(
      tl.x,
      tl.y,
      tr.x,
      tr.y,
  ...
  _computeHomographyDLTFromCoords(tlx, tly, trx, try_, brx, bry, blx, bly) {
    const pts = [
      [0, 0, tlx, tly],
      [1, 0, trx, try_],
      [1, 1, brx, bry],
