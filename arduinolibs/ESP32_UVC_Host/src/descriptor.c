...
static void log_parse_failure(const char *where, int index, int alt, int ep)
{
    ESP_LOGE(TAG, "%s failed while parsing config descriptors (interface=%d alt=%d ep=%d)",
             where, index, alt, ep);
}
...
int parse_configuration(libusb_config_descriptor_t *config, const uint8_t *buffer, int size)
{
    ...
    for (int i = 0; i < config->bNumInterfaces; i++) {
        ...
        if (!ifc_desc) {
            log_parse_failure("interface descriptor lookup", i, -1, -1);
            ret = LIBUSB_ERROR_IO;
            goto cleanup;
        }
        ...
        fprintf(stderr, "descriptor map: config->interface[%d] starts at bInterfaceNumber=%u altCount=%d\n",
                i, ifc_desc->bInterfaceNumber, alt_interfaces);
        ...
        for (int alt = 0; alt < alt_interfaces; alt++) {
            ...
            if (altsetting->bNumEndpoints > 0) {
                LIBUSB_GOTO_ON_FALSE(altsetting->endpoint);
            }

            for (int ep = 0; ep < endpoints; ep++) {
                ep_desc = usb_parse_endpoint_descriptor_by_index(ifc_desc, ep, config->wTotalLength, &offset);
                if (!ep_desc) {
                    log_parse_failure("endpoint descriptor lookup", i, alt, ep);
                    ESP_LOGW(TAG, "truncating endpoint count from %u to %d", endpoints, ep);
                    altsetting->bNumEndpoints = ep;
                    break;
                }
            ...
            if (alt + 1 < alt_interfaces) {
                ...
                ifc_desc = next_interface_desc(ifc_desc, config->wTotalLength, &offset);
                if (!ifc_desc) {
                    log_parse_failure("alternate interface descriptor lookup", i, alt + 1, -1);
                    ret = LIBUSB_ERROR_IO;
                    goto cleanup;
                }
                LIBUSB_GOTO_ON_ERROR( add_extra_data(&extra, ifc_desc) );
