...
void transfer_cb(usb_transfer_t *xfer)
{
    ...
    if (transfer_log_count < 20) {
        fprintf(stderr,
                "libusb_adapter: transfer cb status=%d libusb=%d endpoint=0x%02x packets=%d actual=%u first_packet_status=%d first_packet_bytes=%u\n",
                xfer->status,
                libusb_trans->status,
                libusb_trans->endpoint,
                xfer->num_isoc_packets,
                (unsigned)libusb_trans->actual_length,
                xfer->num_isoc_packets ? xfer->isoc_packet_desc[0].status : -1,
                xfer->num_isoc_packets ? (unsigned)xfer->isoc_packet_desc[0].actual_num_bytes : (unsigned)xfer->actual_num_bytes);
        transfer_log_count++;
    }
...
static esp_err_t wait_for_transmition_done(usb_transfer_t *xfer)
{
    ...
    if (device->transfer_status != USB_TRANSFER_STATUS_COMPLETED) {
        printf("transfer_status: %d endpoint=0x%02x\n", device->transfer_status, xfer->bEndpointAddress);
        return ESP_FAIL;
    }
                            ...
                            unsigned int timeout)
{
    ...
    xfer->timeout_ms = MAX(timeout, 1000);
    ...
    if (control_log_count < 40) {
        fprintf(stderr,
                "libusb_control_transfer: type=0x%02x req=0x%02x value=0x%04x index=0x%04x len=%u timeout=%u\n",
                bmRequestType, bRequest, wValue, wIndex, wLength, timeout);
    }

    esp_err_t submit_err = usb_host_transfer_submit_control(s_uvc_driver->client, xfer);
    if (submit_err != ESP_OK) {
        fprintf(stderr, "libusb_control_transfer submit failed: %s\n", esp_err_to_name(submit_err));
        return esp_to_libusb_error(submit_err);
    }

    esp_err_t wait_err = wait_for_transmition_done(xfer);
    if (control_log_count < 40) {
        fprintf(stderr,
                "libusb_control_transfer done: err=%s status=%d actual=%u\n",
                esp_err_to_name(wait_err), device->transfer_status, (unsigned)xfer->actual_num_bytes);
        control_log_count++;
    }
    if (wait_err != ESP_OK) {
        return esp_to_libusb_error(wait_err);
    }
...
int libusb_get_device_descriptor(libusb_device *dev, struct libusb_device_descriptor *desc)
{
    ...
    fprintf(stderr, "libusb_get_device_descriptor: vid=%04x pid=%04x class=%02x subclass=%02x configs=%u\n",
...
int libusb_set_interface_alt_setting(libusb_device_handle *dev_handle, int32_t inferface, int32_t alt_settings)
{
    ...
    fprintf(stderr, "libusb_set_interface_alt_setting: interface=%ld alt=%ld\n",
            (long)inferface, (long)alt_settings);
    ...
    if (inferface != 0 || alt_settings != 0) {
        esp_err_t release_err = usb_host_interface_release(client, device->handle, inferface);
        fprintf(stderr, "libusb_set_interface_alt_setting: release -> %s\n", esp_err_to_name(release_err));
        if (release_err != ESP_OK) {
            return esp_to_libusb_error(release_err);
        }
        esp_err_t claim_err = usb_host_interface_claim(client, device->handle, inferface, alt_settings);
        fprintf(stderr, "libusb_set_interface_alt_setting: claim alt -> %s\n", esp_err_to_name(claim_err));
        if (claim_err != ESP_OK) {
            return esp_to_libusb_error(claim_err);
        }
        fprintf(stderr, "libusb_set_interface_alt_setting: skipping duplicate SET_INTERFACE after alt claim\n");
        return LIBUSB_SUCCESS;
    }
    ...
    fprintf(stderr, "libusb_set_interface_alt_setting: SET_INTERFACE -> %d\n", result);
