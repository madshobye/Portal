...
#define USB_EP_ISOC_IN_MAX_MPS               384                                         //Max MPS for the custom RX FIFO split used on Arduino-ESP32 3.3.x
...
/********************************************** Helper Functions **********************************************/
#define USB_CTRL_UVC_COMMIT_REQ(ctrl_req_ptr, itf) ({  \
...
#define USB_CTRL_UVC_PROBE_SET_REQ(ctrl_req_ptr, itf) ({  \
        (ctrl_req_ptr)->bmRequestType = 0x21;   \
        (ctrl_req_ptr)->bRequest = UVC_SET_CUR;    \
...
#define USB_CTRL_UVC_PROBE_GET_REQ(ctrl_req_ptr, itf) ({  \
...
#define USB_CTRL_UVC_PROBE_GET_GENERAL_REQ(ctrl_req_ptr, req, itf) ({  \
        (ctrl_req_ptr)->bmRequestType = 0xA1;   \
...
static void _uvc_stream_ctrl_print_summary(const char *label, const uvc_stream_ctrl_t *ctrl)
{
    if (!ctrl) {
        return;
    }
    printf("UVC %s bmHint=0x%04x fmt=%u frame=%u interval=%"PRIu32" maxFrame=%"PRIu32" maxPayload=%"PRIu32" clock=%"PRIu32" iface=%u\n",
           label,
           ctrl->bmHint,
           ctrl->bFormatIndex,
           ctrl->bFrameIndex,
...
IRAM_ATTR static void _processing_uvc_pipe(_uvc_stream_handle_t *strmh, hcd_pipe_handle_t pipe_handle, bool if_enqueue)
{
    ...
    static uint32_t stats_last_ms = 0;
    static uint32_t stats_urbs = 0;
    static uint32_t stats_urb_errors = 0;
    static uint32_t stats_bulk_urbs = 0;
    static uint32_t stats_isoc_packets = 0;
    static uint32_t stats_isoc_bad = 0;
    static uint32_t stats_zero_packets = 0;
    static uint32_t stats_nonzero_packets = 0;
    static uint32_t stats_nonzero_bytes = 0;
    static uint32_t stats_status_counts[USB_TRANSFER_STATUS_NO_DEVICE + 1] = {0};
    static uint32_t stats_first_bad_status = 0;
    static uint32_t stats_first_bad_actual = 0;
    static uint32_t stats_first_bad_requested = 0;
    const bool packet_stats_enabled = s_usb_dev.flags & FLAG_UVC_PACKET_STATS;
    ...
    if (packet_stats_enabled) {
        stats_urbs++;
        if (urb_done->transfer.status != USB_TRANSFER_STATUS_COMPLETED) {
            stats_urb_errors++;
        }
    }
    ...
    if (urb_done->transfer.num_isoc_packets == 0) { // Bulk transfer
        if (packet_stats_enabled) {
            ...
            if (urb_done->transfer.actual_num_bytes > 0) {
                stats_nonzero_packets++;
                stats_nonzero_bytes += urb_done->transfer.actual_num_bytes;
            } else {
                stats_zero_packets++;
            }
        }
        ...
        for (size_t i = 0; i < urb_done->transfer.num_isoc_packets; i++) {
            ...
            if (urb_done->transfer.isoc_packet_desc[i].status != USB_TRANSFER_STATUS_COMPLETED) {
                if (packet_stats_enabled) {
                    stats_isoc_bad++;
                    const usb_transfer_status_t status = urb_done->transfer.isoc_packet_desc[i].status;
                    if (status <= USB_TRANSFER_STATUS_NO_DEVICE) {
                        stats_status_counts[status]++;
                    }
                    if (stats_first_bad_status == 0) {
                        stats_first_bad_status = status;
                        stats_first_bad_actual = urb_done->transfer.isoc_packet_desc[i].actual_num_bytes;
                        stats_first_bad_requested = urb_done->transfer.isoc_packet_desc[i].num_bytes;
                    }
                }
                ESP_LOGV(TAG, "uvc isoc packet=%u status=%d actual=%u",
                         (unsigned)i,
                         urb_done->transfer.isoc_packet_desc[i].status,
                         (unsigned)urb_done->transfer.isoc_packet_desc[i].actual_num_bytes);
                continue;
            }

            uint8_t *simplebuffer = urb_done->transfer.data_buffer + (i * s_usb_dev.uvc->vs_ifc->ep_mps);
            if (packet_stats_enabled) {
                if (urb_done->transfer.isoc_packet_desc[i].actual_num_bytes > 0) {
                    stats_nonzero_packets++;
                    stats_nonzero_bytes += urb_done->transfer.isoc_packet_desc[i].actual_num_bytes;
                } else {
                    stats_zero_packets++;
                }
            }
    ...
    if (packet_stats_enabled) {
        ...
        if (stats_last_ms == 0) {
            stats_last_ms = now_ms;
        } else if ((uint32_t)(now_ms - stats_last_ms) >= 1000) {
            printf("UVC stats urbs=%"PRIu32" urbErr=%"PRIu32" bulk=%"PRIu32" isocPkts=%"PRIu32" bad=%"PRIu32" zero=%"PRIu32" nonzero=%"PRIu32" bytes=%"PRIu32"\n",
                   stats_urbs,
                   stats_urb_errors,
                   stats_bulk_urbs,
                   stats_isoc_packets,
                   ...
                   stats_nonzero_packets,
                   stats_nonzero_bytes);
            printf("UVC status counts ok=%"PRIu32" err=%"PRIu32" timeout=%"PRIu32" cancel=%"PRIu32" stall=%"PRIu32" overflow=%"PRIu32" skipped=%"PRIu32" noDev=%"PRIu32" firstBad=%"PRIu32" actual=%"PRIu32" requested=%"PRIu32"\n",
                   stats_status_counts[USB_TRANSFER_STATUS_COMPLETED],
                   stats_status_counts[USB_TRANSFER_STATUS_ERROR],
                   stats_status_counts[USB_TRANSFER_STATUS_TIMED_OUT],
                   stats_status_counts[USB_TRANSFER_STATUS_CANCELED],
                   stats_status_counts[USB_TRANSFER_STATUS_STALL],
                   stats_status_counts[USB_TRANSFER_STATUS_OVERFLOW],
                   stats_status_counts[USB_TRANSFER_STATUS_SKIPPED],
                   stats_status_counts[USB_TRANSFER_STATUS_NO_DEVICE],
                   stats_first_bad_status,
                   stats_first_bad_actual,
                   stats_first_bad_requested);
            stats_last_ms = now_ms;
        }
    }
...
static esp_err_t _update_config_from_descriptor(const usb_config_desc_t *cfg_desc)
{
    ...
    if (usb_dev->enabled[STREAM_UVC]) {
        const bool force_uvc_config = (usb_dev->flags & FLAG_UVC_FORCE_STREAM_CONFIG) && usb_dev->uvc_cfg.interface;
        if (force_uvc_config) {
            ESP_LOGW(TAG, "Force UVC stream config from user settings");
            UVC_ENTER_CRITICAL();
        ...
        if ((usb_dev->flags & FLAG_UVC_FORCE_STREAM_CONFIG) && usb_dev->uvc_cfg.format_index) {
            ESP_LOGW(TAG, "Force UVC format index = %u", usb_dev->uvc_cfg.format_index);
            uvc_dev->format_index = usb_dev->uvc_cfg.format_index;
        } else if (format_set_found) {
        ...
        if ((usb_dev->flags & FLAG_UVC_FORCE_STREAM_CONFIG) && usb_dev->uvc_cfg.frame_index) {
            ESP_LOGW(TAG, "Force UVC frame index = %u", usb_dev->uvc_cfg.frame_index);
            UVC_ENTER_CRITICAL();
...
static esp_err_t _uvc_vs_commit_control(uvc_stream_ctrl_t *ctrl_set, uvc_stream_ctrl_t *ctrl_probed)
{
    ...
    const uint16_t uvc_vs_interface = s_usb_dev.uvc && s_usb_dev.uvc->vs_ifc
                                      ? s_usb_dev.uvc->vs_ifc->interface
                                      : 1;
    ESP_LOGD(TAG, "UVC control wIndex interface=%u", uvc_vs_interface);
    const bool print_uvc_diagnostics = s_usb_dev.flags & FLAG_UVC_PACKET_STATS;
    if (print_uvc_diagnostics) {
        _uvc_stream_ctrl_print_summary("probe-set", ctrl_set);
    }

    ESP_LOGD(TAG, "SET_CUR Probe");
    xSemaphoreTake(s_usb_dev.xfer_mutex_hdl, portMAX_DELAY);
    USB_CTRL_UVC_PROBE_SET_REQ((usb_setup_packet_t *)urb_ctrl->transfer.data_buffer, uvc_vs_interface);
    ...
    xSemaphoreTake(s_usb_dev.xfer_mutex_hdl, portMAX_DELAY);
    USB_CTRL_UVC_PROBE_GET_REQ((usb_setup_packet_t *)urb_ctrl->transfer.data_buffer, uvc_vs_interface);
    ...
    if (ctrl_probed->dwMaxPayloadTransferSize > ctrl_set->dwMaxPayloadTransferSize) {
        ESP_LOGW(TAG, "Clamp probed payload %"PRIu32" to endpoint payload %"PRIu32,
                 ctrl_probed->dwMaxPayloadTransferSize,
                 ctrl_set->dwMaxPayloadTransferSize);
        ctrl_probed->dwMaxPayloadTransferSize = ctrl_set->dwMaxPayloadTransferSize;
    }
    if (print_uvc_diagnostics) {
        _uvc_stream_ctrl_print_summary("probe-get", ctrl_probed);
    }
    ...
    if (print_uvc_diagnostics) {
        _uvc_stream_ctrl_print_summary("commit-set", ctrl_probed);
    }
    xSemaphoreTake(s_usb_dev.xfer_mutex_hdl, portMAX_DELAY);
    USB_CTRL_UVC_COMMIT_REQ((usb_setup_packet_t *)urb_ctrl->transfer.data_buffer, uvc_vs_interface);
            ...
            case USER_EVENT: {
                ...
                if (evt_msg._event.user_cmd == STREAM_SUSPEND) {
                    ...
                    if (p_itf->urb_list == NULL && p_itf->xfer_type == UVC_XFER_BULK) {
                        ...
                        ESP_LOGD(TAG, "%s stream resume: alloc/enqueue %u urbs, packets=%u, bytes_per_packet=%u",
                                 p_itf->name,
                                 (unsigned)p_itf->urb_num,
                                 (unsigned)p_itf->packets_per_urb,
                                 (unsigned)p_itf->bytes_per_packet);
...
static void _usb_processing_task(void *arg)
{
    ...
    while (1) {
        ...
        if (action_bits & ACTION_DEVICE_CONNECT) {
            ...
            /*
             * Arduino-ESP32 3.3.8 exposes the HCD type declarations used by
             * this vendored usb_stream release, but does not export
             * hcd_port_set_fifo_bias(). The port is already created with a
             * FIFO bias, so keep enumeration moving without this optional
             * post-reset retune.
             */
            action_bits &= ~ACTION_DEVICE_CONNECT;
            action_bits |= ACTION_DEVICE_ENUM;
