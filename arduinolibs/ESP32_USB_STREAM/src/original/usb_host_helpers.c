...
#define USB_PORT_NUM 0  // Arduino-ESP32 3.3.x HCD uses a zero-based root port index.
...
hcd_port_handle_t _usb_port_init(hcd_port_callback_t callback, void *callback_arg)
{
    ...
    static const hcd_fifo_settings_t hcd_fifo_config = {
        .nptx_fifo_lines = 16,
        .ptx_fifo_lines = 12,
        .rx_fifo_lines = 100,
    };
    hcd_config_t hcd_config = {
        ...
        .fifo_config = &hcd_fifo_config,
        .peripheral_map = 1,
    };
    ret = hcd_install(&hcd_config);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "hcd_install returned %s (%d)", esp_err_to_name(ret), ret);
    }
