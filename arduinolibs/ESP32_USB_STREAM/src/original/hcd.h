...
/**
 * @brief Optional user-defined FIFO configuration
 *
 * Arduino-ESP32 3.3.x ships a newer HCD implementation than this vendored
 * usb_stream release expected. Keep this local header ABI-compatible with
 * that implementation so hcd_install() sees the fields it requires.
 */
typedef struct {
...
typedef struct {
    ...
    const hcd_fifo_settings_t *fifo_config; /**< Optional custom FIFO configuration */
    unsigned peripheral_map;                /**< Bit map of USB-OTG peripherals owned by HCD */
} hcd_config_t;
...
typedef struct {
    ...
    unsigned peripheral_idx;                /**< USB peripheral index */
} hcd_port_config_t;
