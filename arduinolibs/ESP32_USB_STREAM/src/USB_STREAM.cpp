...
void USB_STREAM::uvcConfiguration(uint16_t width, uint16_t height, uint32_t frameInterval, uint32_t transferBufferSize, uint8_t *transferBufferA, uint8_t *transferBufferB, uint32_t frameBufferSize, uint8_t *frameBuffer)
{
    ...
    uvcConfiguration(width, height, frameInterval, transferBufferSize, transferBufferA, transferBufferB, frameBufferSize, frameBuffer, options);
}

// Method to configure the uvc camera stream with optional descriptor overrides
void USB_STREAM::uvcConfiguration(uint16_t width, uint16_t height, uint32_t frameInterval, uint32_t transferBufferSize, uint8_t *transferBufferA, uint8_t *transferBufferB, uint32_t frameBufferSize, uint8_t *frameBuffer, const USBStreamUVCOptions &options)
{
    ...
    uvc_config.format = options.format;
    uvc_config.xfer_type = options.xferType;
    uvc_config.format_index = options.formatIndex;
    uvc_config.frame_index = options.frameIndex;
    uvc_config.interface = options.interface;
    uvc_config.interface_alt = options.interfaceAlt;
    uvc_config.ep_addr = options.endpointAddress;
    uvc_config.ep_mps = options.endpointMps;
    uvc_config.flags = options.flags;
