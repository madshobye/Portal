...
struct USBStreamUVCOptions {
    // Leave optional indexes/endpoints as 0 to let the library select them
    // from the camera descriptors. Set them only when testing a specific mode.
    uvc_format_t format = UVC_FORMAT_MJPEG;
    uvc_xfer_t xferType = UVC_XFER_ISOC;
    uint8_t formatIndex = 0;
    uint8_t frameIndex = 0;
    uint16_t interface = 0;
    uint16_t interfaceAlt = 0;
    uint8_t endpointAddress = 0;
    uint32_t endpointMps = 0;
    uint32_t flags = 0;
};

struct USBStreamUVCKnownCameraProfile {
    const char *name;
    uint16_t vendorId;
    uint16_t productId;
    uint16_t width;
    ...
    uint8_t frameIndex;
    uint16_t interface;
    uint16_t interfaceAlt;
    uint8_t endpointAddress;
    uint32_t endpointMps;
};

static constexpr USBStreamUVCKnownCameraProfile USBSTREAM_UVC_LOGITECH_046D_0821_160X120_MJPEG = {
    "Logitech 046d:0821 MJPEG 160x120",
    0x046d,
    0x0821,
    160,
    120,
...
static constexpr USBStreamUVCKnownCameraProfile USBSTREAM_UVC_LOGITECH_046D_0821_320X240_MJPEG = {
    "Logitech 046d:0821 MJPEG 320x240",
    0x046d,
    0x0821,
    320,
    240,
...
static constexpr USBStreamUVCKnownCameraProfile USBSTREAM_UVC_LOGITECH_046D_0821_640X360_MJPEG = {
    "Logitech 046d:0821 MJPEG 640x360",
    0x046d,
    0x0821,
    640,
    360,
...
static constexpr USBStreamUVCKnownCameraProfile USBSTREAM_UVC_LOGITECH_046D_0821_640X480_MJPEG = {
    "Logitech 046d:0821 MJPEG 640x480",
    0x046d,
    0x0821,
    640,
    480,
...
static inline USBStreamUVCOptions USBStreamUVCOptionsForProfile(
    const USBStreamUVCKnownCameraProfile &profile,
    bool packetStats = false
) {
...
public:
    ...
    */
    ...
    void uvcConfiguration(uint16_t width, uint16_t height, uint32_t frameInterval, uint32_t transferBufferSize, uint8_t *transferBufferA, uint8_t *transferBufferB, uint32_t frameBufferSize, uint8_t *frameBuffer, const USBStreamUVCOptions &options);
