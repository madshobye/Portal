...
static int agent_candidate_pair_score(IceCandidatePair* pair) {
  ...
  if (pair->local->type == ICE_CANDIDATE_TYPE_HOST) {
    score += 40000;
...
static int agent_socket_send(Agent* agent, Address* addr, const uint8_t* buf, int len) {
  ...
  LOGD("agent send len=%d to %s:%d family=%d", len, addr_string, addr->port, addr->family);
...
int agent_select_candidate_pair(Agent* agent) {
  int i;
  IceCandidatePair* inprogress_pair = NULL;
  IceCandidatePair* best_frozen_pair = NULL;
  int best_frozen_score = -1;

  for (i = 0; i < agent->candidate_pairs_num; i++) {
    if (agent->candidate_pairs[i].state == ICE_CANDIDATE_STATE_FROZEN) {
      int score = agent_candidate_pair_score(&agent->candidate_pairs[i]);
      if (score > best_frozen_score) {
        best_frozen_score = score;
        best_frozen_pair = &agent->candidate_pairs[i];
      }
    } else if (agent->candidate_pairs[i].state == ICE_CANDIDATE_STATE_INPROGRESS) {
      inprogress_pair = &agent->candidate_pairs[i];
  ...
  if (best_frozen_pair) {
    agent->nominated_pair = best_frozen_pair;
    best_frozen_pair->conncheck = 0;
    best_frozen_pair->state = ICE_CANDIDATE_STATE_INPROGRESS;
    {
      char local_addr[ADDRSTRLEN];
      char remote_addr[ADDRSTRLEN];
      addr_to_string(&best_frozen_pair->local->addr, local_addr, sizeof(local_addr));
      addr_to_string(&best_frozen_pair->remote->addr, remote_addr, sizeof(remote_addr));
      LOGI("selected ICE pair local=%s:%d remote=%s:%d score=%d",
           local_addr,
           best_frozen_pair->local->addr.port,
           remote_addr,
           best_frozen_pair->remote->addr.port,
           best_frozen_score);
    }
    return 0;
  }
