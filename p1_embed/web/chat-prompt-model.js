export function buildSpecificationGeneratePrompt(specification, {
  ...
  return [
    ...
    "Generate a complete XOBIT Wrench script from the project specification below.",
...
export function buildChatInstructions(context) {
  return [
    "You are the XOBIT Wrench coding assistant inside a browser tool for an ESP32 classic firmware.",
    ...
    "Avoid factory reset or destructive device actions. Do not invent firmware bindings beyond the documented XOBIT bindings.",
...
export function buildChatRequestPayload({
  ...
  const body = {
    ...
    text: {
      format: {
        type: "json_schema",
        name: "xobit_wrench_assistant_response",
        strict: false,
