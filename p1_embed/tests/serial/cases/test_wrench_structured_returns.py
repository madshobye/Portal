...
def test_rgb_hsv_scalar_bindings_with_2000_led_buffer_no_modulo(dev):
    reboot_and_wait(dev)
    code = """
var ops = 0;
var checksum = 0;
...
function setup() {
  ...
  minHeap = startHeap;
  maxHeap = startHeap;
  println("rgb hsv scalar 2000 start=" + startHeap);
}
...
function loop() {
  if (done == 0) {
    ...
    while (j < 100 && ops < ledTotal) {
      ...
      var ng = hsvToG(h, s, v);
      var nb = hsvToB(h, s, v);
      ledSet(0, ops, nr, ng, nb);
      checksum = checksum + nr + ng + nb + h + s + v;
    ...
    if (ops >= ledTotal) {
      ...
      println("rgb hsv scalar 2000 done ops=" + ops + " checksum=" + checksum + " start=" + startHeap + " min=" + minHeap + " max=" + maxHeap + " end=" + freeHeap());
    }
  }
  delay(1);
}
""".strip()
    ...
    dev.command("script.set", {"code": code, "run": True, "save": False}, timeout=14.0)
    start_event = dev.wait_event("script.print", timeout=6.0)
    assert_true(start_event.get("data", {}).get("message", "").startswith("rgb hsv scalar 2000 start="), "2000 LED scalar diagnostic should start")
    done_event = dev.wait_event("script.print", timeout=18.0)
    message = done_event.get("data", {}).get("message", "")
    assert_true(message.startswith("rgb hsv scalar 2000 done ops="), "2000 LED scalar diagnostic should finish")
    values = parse_diag_numbers(message)
    assert_equal(values.get("ops"), 2000, f"2000 LED scalar operation count: {message}")
    assert_true(values.get("end", 0) >= values.get("start", 0) - 2000, f"2000 LED scalar heap should recover within running script: {message}")

    last = dev.command("script.error.get", timeout=4.0)
    assert_equal(last.get("hasError"), False, "2000 LED scalar diagnostic should not leave an error")
    reboot_and_wait(dev)
    after = dev.command("status.get", timeout=4.0).get("freeHeap", 0)
    assert_true(after > before - 18000, f"free heap should recover after rebooting 2000 LED scalar script before={before} after={after}")
