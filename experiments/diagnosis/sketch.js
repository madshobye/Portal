...
const DOC_MD_URL = "./doctorprompt.md";
const MODEL_OPTIONS = [
  "gpt-5.4",
  "gpt-5.4-pro",
  "gpt-5.4-nano",
...
const HISTORY_LIMIT_KEY = `${STORAGE_PREFIX}.historyLimit`;
...
const CHAT_HISTORY_LIMIT_OPTIONS = [6, 10, 14, 20, 30, 40, 50];
...
const VOICE_STARTUP_CHECK_MS = 1400;
const VOICE_STARTUP_RECOVERY_DELAY_MS = 320;
const VOICE_STARTUP_MAX_RECOVERY_ATTEMPTS = 2;
...
let selectedVoice = "";
let chatHistoryLimit = DEFAULT_CHAT_HISTORY_LIMIT;
...
let voiceStartupCheckTimeout = null;
let voiceStartupRecoveryCount = 0;
...
const structuredSchemas = [
  {
    name: "doctor_reply",
    description: "Return the doctor interview answer and diagnosis metadata.",
    parameters: {
      type: "object",
      properties: {
        reply: { type: "string" },
...
async function setup() {
  ...
  chatHistoryLimit = loadChatHistoryLimit();
...
function buildUi() {
  ...
  historyLimitSelectEl = refs.historyLimitSelectEl;
  ...
  for (const limit of CHAT_HISTORY_LIMIT_OPTIONS) {
    historyLimitSelectEl.option(`History: ${limit}`, String(limit));
  }
  historyLimitSelectEl.selected(String(chatHistoryLimit));
  historyLimitSelectEl.changed(() => {
    const nextLimit = sanitizeChatHistoryLimit(historyLimitSelectEl.value());
    chatHistoryLimit = nextLimit;
    persistChatHistoryLimit();
    trimChatHistory();
    appendSystemMessage(`Chat history limit changed to ${chatHistoryLimit}.`);
  });
...
function measureStageBubbleLayout(entry, columnX, columnWidth, fitScale = 1) {
  ...
  const isProcessing = entry.kind === "processing";
...
function drawStageBubbleLayout(layout, y, options = {}) {
  ...
  textAlign(LEFT, TOP);
  fill(layout.isRecording ? 255 : 247, layout.isRecording ? 255 : 245, layout.isRecording ? 255 : 239, 255);

  let textX = layout.bubbleX + layout.paddingX;
  if (layout.isRecording) {
    drawRecordingIndicator(textX + 10, y + layout.bubbleH * 0.5);
    textX += 36;
  } else if (layout.isProcessing) {
    drawProcessingIndicator(textX + 10, y + layout.bubbleH * 0.5);
    textX += 34;
  }
...
function drawRecordingIndicator(cx, cy) {
  ...
  fill(216, 31, 38, 34 + 60 * glow);
  circle(cx, cy, 22 * pulse);
  fill(255, 88, 102, 255);
  circle(cx, cy, 12.5);

  noFill();
  stroke(255, 94, 108, 70 + 120 * glow);
  strokeWeight(1.8);
  circle(cx, cy, 20 + 5 * pulse);
  stroke(255, 94, 108, 28 + 52 * glow);
  strokeWeight(1.2);
  circle(cx, cy, 28 + 8 * pulse);
}

function drawProcessingIndicator(cx, cy) {
  const pulse = 0.55 + 0.45 * (0.5 + 0.5 * Math.sin(millis() * 0.012));
  noStroke();
  fill(255, 253, 248, 210);
  for (let i = 0; i < 3; i += 1) {
    const alpha = 50 + 160 * Math.max(0.15, Math.sin(millis() * 0.01 + i * 0.7) * 0.5 + 0.5);
    fill(255, 253, 248, alpha);
    circle(cx + i * 10, cy, 7 + 3 * pulse);
  }
}
...
function describeFaceState() {
  return currentMood?.label || "caring";
}
...
function createClient() {
  return new GptClient({
    ...
    model: selectedModel,
    instructions:
      "You are a warm, attentive doctor interviewing a patient to understand their symptoms and eventually provide a likely diagnosis. Always respond through the doctor_reply function.",
...
function toggleListening() {
  ...
  if (listeningWanted || speech.isListening() || isSpeechOutputActive()) {
    ...
    if (!conversationStarted) {
      ...
      startVoiceListening("manual toggle");
    }
  }
  if (listenButton) {
    listenButton.html(
...
function initPrestartVoiceMode() {
  ...
  try {
    ...
    speech.stopListening();
    voiceStartupRecoveryCount = 0;
    startVoiceListening("prestart arm");
    hasSessionInteraction = true;
...
function buildConversationPrompt(latestUserMessage) {
  const fallbackPrompt = [
    ...
    "",
    "You are a warm, careful doctor speaking with a patient.",
    "Ask one focused follow-up question at a time and work toward a likely diagnosis.",
    "Keep responses fairly short. Stay in character.",
    "This is a roleplay interview, not real medical care.",
    "",
    "## Session summary",
    "[session_summary]",
    "",
  ...
  const docText = injectPromptPlaceholders(baseDocText, {
    ...
    session_summary: sessionSummary || "No clear pattern yet. Keep assessing the patient carefully.",
  });
  ...
  return [
    ...
    "Do not invent missing clinical details or change the patient's intent.",
    "",
    "Respond as the doctor using the doctor_reply function.",
    "reply: the doctor's in-character reply only.",
    "trainee_assessment: a short clinical read of what the patient's latest answer suggests.",
    "next_focus: one short phrase describing what the doctor should clarify next.",
    "cleaned_trainee_message: rewrite the patient's latest message into a short, clean version that preserves intent and fixes obvious speech-to-text mistakes, dropped words, and garbled phrasing. If the message is already clear, return it with only light cleanup.",
    "task: the current diagnostic goal in one or two short sentences. Update it as the interview progresses.",
    "options: optional short patient reply ideas as a list of 0 to 4 strings. Use them only when genuinely helpful.",
    "mood: assess the doctor avatar mood and return label, valence (-1 to 1), arousal (-1 to 1), dominance (-1 to 1), and tension (0 to 1). Keep the doctor generally warm, attentive, and composed.",
    "silence_prompts: exactly 3 short doctor follow-up lines that become slightly more direct if the patient stays silent. Keep them brief, kind, and suitable for pacing delays.",
  ].join("\n");
}
...
function buildOpeningPrompt() {
  const fallbackPrompt = [
    ...
    "",
    "You are a warm, careful doctor speaking with a patient.",
    "Start with a kind welcome, ask how the patient is today, and invite them to describe what is bothering them.",
    "Do not wait for a patient message before starting.",
    "",
    "## Session summary",
    "Session start.",
    "",
  ...
  return [
    ...
    "Open warmly, ask how the patient is today, and invite them to explain their problem.",
    "Do not mention metadata or explain the rules.",
    "",
    "Respond as the doctor using the doctor_reply function.",
    "reply: the doctor's in-character opening line only.",
    "trainee_assessment: use 'Session start'.",
    "next_focus: use a short phrase for the first thing the doctor should clarify.",
    "cleaned_trainee_message: return '(none)' because there is no patient message yet.",
    "task: define the current diagnostic goal in one or two short sentences.",
    "options: provide 2 to 4 short possible patient replies or symptom descriptions.",
    "mood: set the initial doctor avatar mood with label, valence (-1 to 1), arousal (-1 to 1), dominance (-1 to 1), and tension (0 to 1).",
    "silence_prompts: provide exactly 3 gentle escalating lines to prompt the patient if they stay silent.",
  ].join("\n");
}
...
function buildConversationHistoryText() {
  return chatHistory
    ...
    .map((item, index) => `${index + 1}. ${item.role === "assistant" ? "Doctor" : "Patient"}: ${item.text}`)
    .join("\n");
}
...
function buildSessionSummary() {
  ...
  if (traineeTurns.length) {
    summary.push(`Recent patient focus: ${traineeTurns.slice(-2).join(" | ")}`);
  }

  const lastNurseMeta = nurseTurns.slice(-2).join(" | ");
  if (lastNurseMeta) {
    summary.push(`Recent doctor direction: ${lastNurseMeta}`);
  }
...
function sanitizeChatHistoryLimit(value) {
  const numeric = Number(value);
  if (CHAT_HISTORY_LIMIT_OPTIONS.includes(numeric)) return numeric;
  return DEFAULT_CHAT_HISTORY_LIMIT;
}

function loadChatHistoryLimit() {
  try {
    return sanitizeChatHistoryLimit(window.localStorage.getItem(HISTORY_LIMIT_KEY));
  } catch {}
  return DEFAULT_CHAT_HISTORY_LIMIT;
}

function persistChatHistoryLimit() {
  try {
    window.localStorage.setItem(HISTORY_LIMIT_KEY, String(chatHistoryLimit));
  } catch {}
}
...
function applySessionLanguage() {
  ...
  if (listeningWanted || speech.isListening()) {
    try {
      ...
      voiceStartupRecoveryCount = 0;
      if (listeningWanted) {
        startVoiceListening("session language change");
      }
    } catch (err) {
...
function maybeReturnToStartForNoResponse() {
  ...
  appendAdminLog("No patient response for 1.5 minutes. Returning to start screen.");
  clearSilencePromptTimers();
  cancelListeningRestart();
  cancelVoiceStartupCheck();
  clearNoResponseTimeoutState();
...
function scheduleListeningRestart(delayMs = VOICE_RECOGNITION_RESTART_DELAY_MS) {
  ...
  listeningRestartTimeout = window.setTimeout(() => {
    ...
    try {
      voiceStartupRecoveryCount = 0;
      startVoiceListening("post-speech resume");
    } catch (err) {
...
function startVoiceListening(reason = "manual") {
  if (!speech || !listeningWanted) return false;
  speech.listenRecurring(null, {
    language: selectedSessionLanguage,
    interimResults: true,
  });
  appendAdminLog(`Voice: listening started (${reason})`);
  scheduleVoiceStartupCheck(reason);
  return true;
}

function scheduleVoiceStartupCheck(reason = "manual") {
  if (!speech || !listeningWanted) return;
  cancelVoiceStartupCheck();
  voiceStartupCheckTimeout = window.setTimeout(async () => {
    voiceStartupCheckTimeout = null;
    if (!speech || !listeningWanted || isSpeechOutputActive()) return;
    if (speech.isListening()) {
      voiceStartupRecoveryCount = 0;
      return;
    }
    if (voiceStartupRecoveryCount >= VOICE_STARTUP_MAX_RECOVERY_ATTEMPTS) {
      appendAdminLog(`Voice recovery exhausted (${reason})`);
      return;
    }
    voiceStartupRecoveryCount += 1;
    appendAdminLog(`Voice recovery ${voiceStartupRecoveryCount}: reinitialising recognizer (${reason})`);
    try {
      await speech.reinitializeRecognition({ restartIfNeeded: false });
      applySelectedVoice(false);
      await new Promise((resolve) => window.setTimeout(resolve, VOICE_STARTUP_RECOVERY_DELAY_MS));
      startVoiceListening(`recovery ${voiceStartupRecoveryCount}`);
    } catch (err) {
      appendAdminLog(`Voice recovery error: ${err?.message || String(err)}`);
    }
  }, VOICE_STARTUP_CHECK_MS);
}

function cancelVoiceStartupCheck() {
  if (voiceStartupCheckTimeout !== null) {
    clearTimeout(voiceStartupCheckTimeout);
    voiceStartupCheckTimeout = null;
  }
}
...
function exportDebugTurn({ latestUserMessage, prompt, result, phase }) {
  ...
  const payload = {
    ...
    function_name: gpt?.functionName || "doctor_reply",
