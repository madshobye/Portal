...
/*			-minBracket	-tolerance	color_hsl	+tolerance	+maxBracket
		...
		isf_FragColor = src_rgb;
		//isf_FragColor = vec4(src_rgb.a, src_rgb.a, src_rgb.a, 1.0);
	}
		...
		isf_FragColor = vec4(src_rgb.rgb, maxAlpha);
	}
	else if (PASSINDEX==2)	{
		...
		isf_FragColor = vec4(srcColor.r, srcColor.g, srcColor.b, totalAlpha);
	}
	else if (PASSINDEX==4)	{
		float		tmpRadius = float(int(blur));
		vec4		tmpColor;
		...
		if (showAlpha)	{
			isf_FragColor = vec4(totalAlpha, totalAlpha, totalAlpha, 1.0);
		}
		else	{
			if (applyAlpha)
				isf_FragColor = vec4(srcColor.r*totalAlpha, srcColor.g*totalAlpha, srcColor.b*totalAlpha, 1.0);
			else
				isf_FragColor = vec4(srcColor.r, srcColor.g, srcColor.b, totalAlpha);
		}
	}
