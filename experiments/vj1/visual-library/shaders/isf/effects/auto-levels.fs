...
void main()
{
	...
	if (PASSINDEX == 0)	{
		...
		isf_FragColor = vec4(vals.x,vals.y,vals.z,min_color.a);
	}
	...
	//	read from "bufferPassD", write to "bufferVariableNameA"
		...
		vals = minmaxcompare(min_colorLB,vals);
		vals = minmaxcompare(min_colorRB,vals);
		isf_FragColor = mix(vec4(vals.x,vals.y,vals.z,min_color.a),stalePixel,adapt_rate);;
	}
	//	read from "bufferVariableNameA", write to output
		...
		isf_FragColor = final_pixel;
		//isf_FragColor = vec4(vec3(vals.z),1.0);
	}
}
