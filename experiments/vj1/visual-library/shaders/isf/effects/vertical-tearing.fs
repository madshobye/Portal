...
void main()
{
	...
	if (PASSINDEX == 0)	{
		isf_FragColor = IMG_NORM_PIXEL(newImage,isf_FragNormCoord.xy);
	}
	else if (PASSINDEX == 1)	{
		isf_FragColor = IMG_NORM_PIXEL(inputImage,isf_FragNormCoord.xy);
	}
	else if (PASSINDEX == 2)	{
		...
		isf_FragColor = (isf_FragNormCoord.y > tearPosition) ? freshPixel : stalePixel;
	}
}
