...
void main() {
	...
	//	first three passes are just copying the input image into the buffer at in sizes
	if (PASSINDEX==0)	{
		isf_FragColor = IMG_NORM_PIXEL(inputImage, isf_FragNormCoord);
	}
	else if (PASSINDEX==1)	{
		isf_FragColor = IMG_NORM_PIXEL(halfSizeBaseRender, isf_FragNormCoord);
	}
	else if (PASSINDEX==2)	{
		isf_FragColor = IMG_NORM_PIXEL(quarterSizeBaseRender, isf_FragNormCoord);
	}
	...
	//	we increase the resolution & blur (the lower-res blurred image from the previous pass) again...
		...
		//isf_FragColor = vec4((sample0 + sample1 + sample2).rgb / (3.0), 1.0);
		//isf_FragColor = vec4((sample0 + sample1 + sample2 + sample3 + sample4).rgb / (5.0), 1.0);
		isf_FragColor = vec4((sample0 + sample1 + sample2 + sample3 + sample4).rgba / (5.0));
	}
	else if (PASSINDEX == 4)	{
	...
	//	...writes into the full-size
		...
		//isf_FragColor =  vec4((sample0 + sample1 + sample2).rgb / (3.0), 1.0);
		isf_FragColor =  vec4((sample0 + sample1 + sample2).rgba / (3.0));
	}
	else if (PASSINDEX == 6)	{
		...
		isf_FragColor = max(mix(originalImg,blurredImg*1.2,intensity), originalImg);
	}
}
