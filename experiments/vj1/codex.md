...
VJ1 is a build-free browser VJ, visual-component, and projection-mapping app in `experiments/vj1`. It uses p5.js and raw WebGL. A user-selected local folder is authoritative for `project.json`, media, shaders, revisions, and derived cache files; mappings and scenes are canonical data inside `project.json` rather than separate scaffold folders.
...
- v19: removes Component and Canvas-frame thumbnails from canonical project data; thumbnails live in `vj1-cache/thumbnails` and regenerate on demand.

The local project folder owns project state. `localStorage` is never authoritative. Completed user transactions enter a serialized save queue at the next task boundary; non-historical UI metadata remains debounced by 700 ms. Revision files implement undo/redo, and a scrub/edit gesture records only its completed commit. UI-only state and metrics are excluded from history signatures. Folder refresh and media import update the authoritative asset snapshot without reloading `project.json`; otherwise an older disk snapshot can replace newer edits or valid selections.

An empty project creates `media/`, `shaders/`, `scenes/`, `mappings/`, and `vj1-cache/renditions/`.

## Components, Canvas, and Scenes

A Component has exactly one graph authority: `component.chain`, containing source, effect, and Group items. Disabled items are skipped. `component.source`, `component.shaderChain`, and chain-level source parameters do not exist at runtime and must never be recreated as mirrors or fallbacks. The v17→v18 project migration consumes those historical fields and removes them before normalization; malformed current-schema source nodes fail with a structured `VJ1_INVALID_*` error instead of silently becoming Test Pattern or black.
...
Deferred control renders are requests, not state authorities: when their animation frame runs they consume the newest store snapshot. Identity-changing structural events—including Component, chain-item, and Surface selection—cancel pending deferred renders and force the next-frame inspector rebuild even while an old control still owns focus. Persistent slider and transform scrubs are one transaction with a single pre-gesture baseline, lightweight intermediate updates, and one normalized/history-aware release commit. This is the UI form of user truth; an older captured render snapshot must never reverse the latest command or retain an obsolete target ID.
...
Large STL/OBJ files are reduced off the render thread with the vendored meshoptimizer 0.25 WebAssembly simplifier. OBJ parsing retains its compact source vertex/index topology; STL triangle soup is welded once into the same indexed contract. Automatic LODs form one progressive, topology-following quadric-error sequence over the indexed mesh—never grid-cluster or recursively simplify an emitted triangle soup. Closed manifold input must remain closed and manifold at every LOD. `renderQuality`, presented as Geometry detail for models, selects among these prebuilt levels without reparsing or rebuilding the model. The resulting typed LOD package is a versioned derived artifact in `vj1-cache/models`, keyed by source revision, model type, simplifier version, and LOD recipe. Preview and output contexts open that exact filename through the persisted project-directory handle; they never enumerate the model-cache directory. A cache miss performs one worker job and writes atomically for later sessions, concurrent identical requests share their in-flight job, and an expired worker job must fail with a structured diagnostic rather than leave media permanently loading. Media-library thumbnails are a separate bounded sampler: they retain at most 600 representative triangles and must never invoke QEM/LOD generation merely because a model card becomes visible. Keep meshoptimizer's MIT license beside the vendored module and report cache failures, simplification error, or topology-limited levels through structured `VJ1` diagnostics.
...
The selected Live Component separates its public performance surface from its implementation. Controls contains a temporary root transform (position, scale, and rotation), Component opacity, speed, and blend, plus parameters and element transforms explicitly published through `significantParams`; Elements contains the nested chain outline and the selected element's Primary, Details, and Transform editor. The root transform is applied to the completed full-frame Component texture as one optional GPU pass, so it never aliases or rewrites the first chain element. Do not flatten every internal element parameter into the Component Controls view.
...
Project-folder reconciliation is never a render-time polling task. Opening a project inventories only root-level supported assets plus the `media` and `shaders` trees. Renditions use a bounded cache manifest and exact file opens; discovery never enumerates a cache directory. Explicit imports publish their already-known file handles directly. The Choose Media modal owns the deliberate full refresh; focus and visibility changes do not scan. If `FileSystemObserver` is available, exact changed paths receive targeted add/change/remove handling, while unknown or directory-wide changes request manual refresh. Never recurse into `revisions` or `vj1-cache`, and never reopen or hash every file merely to detect a change. Traversal and cleanup loops yield cooperatively.

Undo history records user-authored transactions, not emissions. Scrub/edit samples retain one pre-gesture baseline and the commit produces one checkpoint. UI selection, activity timestamps, calibration state, metrics, thumbnails, and other derived cache changes may be persisted when appropriate but never create undo entries. History equality uses canonical key ordering. The revision service indexes its directory once per project session, updates that index incrementally after saves, and bounds undo and redo independently to 500 files and 512 MB; ordinary saves never enumerate the revisions directory.

Rendered thumbnails are derived cache resources. `project.json` serialization must strip Component `thumbnail` and Canvas `frameThumbnails`; the thumbnail store owns deterministic WebP/PNG files and object-URL disposal. Thumbnail capture updates use the derived-state channel, which does not autosave, stamp activity, rebuild Live state, or enter history.
