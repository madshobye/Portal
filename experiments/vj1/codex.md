...
`project.json` uses a strict sequential schema version defined by `CURRENT_PROJECT_VERSION` in `js/domain/project-migrations.js`; the current format is version 11. Increment this version whenever the persisted project data model changes. Each increment must add one adjacent named migration and register it by its source version: version 6→7 is a separate function from 7→8. The migration runner always applies every intervening step in order, so a version 5 project opened by a version 11 application executes 5→6→7→8→9→10→11 and fails safely if any step is missing. Migrations run before `sanitizeState()`; sanitization then supplies defaults, clamps values, and repairs references, but it is not the version history. Unversioned legacy files enter at version 1. Invalid or newer-than-supported project files are not loaded or autosaved, preventing an older application from overwriting an unknown format. To introduce a new format: bump `CURRENT_PROJECT_VERSION`, add and register exactly one adjacent migration, update current fixtures, and add focused migration tests.
...
Version 12 makes Canvas Component placement independent from Component texture resolution. Migration 11→12 records each existing referenced Component's current footprint as normalized Canvas-relative width and height, including references nested in Groups and old `canvas.layers`. New references capture that normalized footprint when they are added. Later texture-resolution changes therefore resize render buffers but do not traverse the project or change existing Canvas layouts.

Version 13 makes the referenced Component authoritative for its dimensions and aspect. Migration 12→13 reduces the temporary two-axis Canvas footprint to one normalized placement scale. Canvas retains that scale plus its existing position and rotation transform; proportional resolution changes preserve placement size, while intentional aspect changes reshape the placement from the Component without stretching it into an obsolete Canvas rectangle.
...
- `render`: ordered projector output definitions, shared mapping-world size, independent component dimensions, adaptive surface-texture policy, pixel density, edge softness, camera preferences, component upscaling, and full-resolution post-filter settings.
...
`renderCanvasComponent()` evaluates the shared chain into one GPU target whose physical size comes from the current demand request while transforms and recording frames retain Canvas-logical coordinates. A routed frame is sampled from that adaptively sized Canvas raster into an adaptively bounded surface texture before projection mapping. Large logical Canvas dimensions therefore do not require a same-sized GPU texture unless Full-quality preview or a sufficiently large mapped route actually demands it.
...
| Ordinary Component design frame | `render.componentTexture` plus `component.frameShape` | Logical aspect and base dimensions. |
| Ordinary Component raster ceiling | `render.pixelDensity × component.resolutionScale` | Maximum physical raster request; it does not change logical geometry. |
...
| Surface sampling policy | `render.surfaceTexture` | Auto follows mapped demand; Manual additionally caps surface width and height. It does not define Component or Canvas dimensions. |
...
```text
state -> needed components -> component buffers -> surface textures
...
8. Component design dimensions come from `render.componentTexture`; the independent `render.surfaceTexture` policy only controls whether per-surface demand is automatic or manually capped. Pixel density supersamples the demand path.
9. Component previews use the current preview frame or requested texture resolution, not the popup window's dimensions.
...
11. Canvas containers render at their logical dimensions so recording-frame coordinates remain exact; their referenced components can still reuse the normal component cache.
...
- **Component texture size** defines an ordinary Component's own landscape frame, aspect, and maximum native detail at 1× resolution. Canvas references snapshot one normalized scale when inserted: proportional resolution changes preserve placement size, while aspect changes follow the Component rather than stretching it.
- **Surface texture size** is demand-driven. `render.surfaceTexture.mode: "auto"` allocates each visible surface route from its projected footprint. `"manual"` activates `maxWidth`/`maxHeight` only as final surface ceilings and never changes component geometry. Pixel density multiplies projected demand instead of serving as an escape from a hidden low-resolution cap.
...
- **Popup window size** controls the HTML/p5 canvas display, not component texture quality.
...
Render demand accepts a generic `samplingScale` supplied by the source-view contract. A routed Canvas recording frame declares 1.5× because it passes through whole-Canvas accumulation, frame cropping, surface resampling, and projective filtering. This raises the Canvas and every referenced component/effect upstream as one demand graph; whole-Canvas routes remain at 1×. Keep this requirement in the source view rather than adding Canvas branches to the geometry planner.
...
Project Settings is a tabbed, mount-once modal with Outputs, Camera, and Rendering views. State changes patch existing controls in place instead of regenerating the modal HTML. Only the configured-output card list may be rebuilt when its output identity structure changes. Settings must not use scroll capture or scroll restoration to compensate for DOM replacement.
...
Mapped rendering uses one generic demand path for ordinary components and Canvas recording-frame sources. `componentSourceView()` describes logical source size and the sampled rectangle; `sourceRenderDemand()` intersects the mapped quad with the current renderer viewport and derives the physical component raster plus final surface raster. Multiple routes sampling one component share the largest required component raster for that renderer/frame. Routes outside an output viewport are culled before source rendering or surface allocation. Canvas is special only when resolving its recording-frame sample rectangle; crop, demand, cache, and projection code remain shared.

Logical component/Canvas dimensions must never be reduced to improve performance. Physical raster demand is allowed to vary by preview size, projector viewport, mapped footprint, and nested placement. Component references inherit the pixel demand of their placement and remain capped by the referenced component's configured maximum. Static caching follows the complete component dependency graph, including Canvas dimensions and nested media; thumbnail data is excluded from render signatures. A static Canvas must be cacheable under the same rules as a static ordinary component.
...
Runtime profiles expose `surfaceRouteCandidates`, `surfaceRoutesVisible`, `surfaceRoutesCulled`, `componentRasterPixels`, `surfaceRasterPixels`, `directSourceComposites`, and `avoidedSourceRasterPixels`. Use these with wall-clock frame time and FPS to verify that an optimization removes planned work. The GPU readout is an averaged query signal and must not be interpreted as total frame GPU time.
