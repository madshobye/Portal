...
- The 2026-07-22 full automated run passes **856/856**. Focused metrics pass **10/10**, focused render geometry passes **23/23**, and `git diff --check` is clean. Contracts cover multiple outputs, popup aspect changes, relative Surface proportions, direct-frame reuse, intrinsic Component placement, hidden-element hit testing, preview panning, Live filters, restored Scene selection, and ISF invalid-definition de-duplication. File-backed nodes may be unresolved between the lightweight project snapshot and asset scan: UI and rendering treat them as pending/dynamic, and graph compilation temporarily represents a pending source as transparent or a pending effect as pass-through. Their IDs never enter the strict built-in catalog and the temporary result is never cached as stable; the same graph activates when asset definitions arrive. The real `mappertest` project also passes an in-memory compact save/load/save equivalence check.
