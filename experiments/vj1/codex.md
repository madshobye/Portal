...
Imported-video decode and playback have separate owners. Loading configures a muted, default-muted, inline, preloaded browser element but never calls p5 `loop()` or starts playback. The active render source alone owns loop/segment state, rate, pause, and play. Promise and synchronous playback rejection emit one `[VJ1_VIDEO_PLAYBACK_FAILED]` diagnostic per element; they are never silently swallowed.
...
The handover reports **419 passing Node tests**. Imported images, live camera capture, Scene projection mapping, and Component/Canvas selection, move, scale, and rotation controls have been confirmed in the running app. The transform-editor fix makes p5 logical canvas dimensions authoritative for pointer conversion instead of DOM backing-store dimensions. Real GLSL, imported-video playback, and cross-window projection parity still require live smoke evidence; Feature Morph has its own inference-and-shader smoke fixture at `tests/browser/feature-morph-smoke.html`. The representative performance comparison is `metrics-results/runs/four-surface-show-gpu-architecture.*`.
