...
Project-folder reconciliation is never a render-time polling task. The app refreshes after explicit import and when focus or visibility returns; the Choose Media modal exposes the deliberate manual refresh action. Do not restore a periodic recursive `getFile()` scan: exact size/modified comparison materializes a `File` for every directory entry and can interrupt Preview cadence in large projects.
