...
test("heartbeat pulse exposes double-beat radial distortion controls", () => {
  ...
  assert.deepEqual(ids, ["renderQuality", "amount", "pulse", "ringWidth", "spread"]);
  const pulse = component.params.find((param) => param.id === "pulse");
  assert.equal(pulse.defaultAnimation.transportKind, "envelope");
  assert.equal(pulse.defaultAnimation.triggerKind, "periodic");
  assert.equal(pulse.defaultAnimation.envelopeSegments.length, 5);
  assert.ok(component.code.includes("float beat = clamp(pulse"));
  assert.ok(!component.code.includes("beatTime"));
  assert.ok(!component.code.includes("uniform float time"));
