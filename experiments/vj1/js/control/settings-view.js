  ...
  return `
    ...
    <section class="modal-panel settings-modal" data-settings-modal role="dialog" aria-modal="true" aria-label="Project settings">
      ...
      <div class="settings-modal-body">
        <section class="element-section" data-settings-panel="outputs" ${visiblePanel("outputs", activeTab)}>
        ...
        <section class="element-section" data-settings-panel="camera" ${visiblePanel("camera", activeTab)}>
        ...
        <section class="element-section" data-settings-panel="rendering" ${visiblePanel("rendering", activeTab)}>
...
function percentRange(label, path, value, min, max, step, marker) {
  return `
    <label class="field range-field">
      <span data-${marker}-label>${label} · ${Math.round(value * 100)}%</span>
      <input type="range" min="${min}" max="${max}" step="${step}" data-settings-update="${path}" value="${value}" />
    </label>
  `;
}
