...
export function compileSceneRenderPrograms(state = {}, groups = []) {
  const surfacesById = new Map((state.surfaces || []).map((surface) => [String(surface.id || ""), surface]));
  const programs = new Map();
  for (const group of groups || []) {
    if (group.generatedBy !== SCENE_PROGRAM_GENERATOR || group.nodeId !== SceneProgramNode.id) continue;
    const surfaceIds = (group.nodes || []).filter((node) => node.role === "surface-route").map((node) => node.surfaceId);
    programs.set(group.sceneId || "", Object.freeze({
      id: group.id,
      sceneId: group.sceneId || "",
      surfaces: Object.freeze(surfaceIds.map((id) => surfacesById.get(id)).filter(Boolean)),
      generatedBy: SCENE_PROGRAM_GENERATOR,
    }));
  }
  return programs;
...
export function activeSceneProgramSurfaces(state = {}, programs = new Map()) {
