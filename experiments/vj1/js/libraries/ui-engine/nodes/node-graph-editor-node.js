...
export const NodeGraphEditorNode = defineUiNode({
  id: "core.ui.node-graph-editor",
  name: "Node Graph Editor",
  version: "0.1.0",
  description: "Retained interactive graph editor. It owns graph DOM and emits semantic graph editing commands.",
  inlets: {
    definition: { type: "any" },
    options: { type: "any", optional: true },
  },
  outlets: {
    graphChange: { type: "event", optional: true },
    status: { type: "event", optional: true },
    mediaRequest: { type: "event", optional: true },
    publicParameterToggle: { type: "event", optional: true },
    publicPortToggle: { type: "event", optional: true },
  },
  events: ["graph-change", "status", "media-request", "public-parameter-toggle", "public-port-toggle"],
  capabilities: ["ui-editor", "ui-node-graph-editor"],
  factory: createNodeGraphEditorInstance,
});

export const NodeDefinitionStudioNode = defineUiNode({
  id: "core.ui.node-definition-studio",
  name: "Node Definition Studio",
  version: "0.1.0",
  description: "Semantic node-definition overview with a retained graph-editor slot.",
  inlets: {
    definition: { type: "any" },
  ...
  outlets: {},
  capabilities: ["ui-display", "ui-container", "ui-node-definition-studio"],
  factory: createNodeDefinitionStudioInstance,
});

function createNodeDefinitionStudioInstance({ host, inputs: initialInputs, document }) {
  ...
  function mount() {
    ...
    root.className = "node-library-workspace ui-node-definition-studio";
    root.dataset.uiNodeOwned = "node-definition-studio";
  ...
  function update(nextInputs = {}) {
    ...
    if (!definition) {
      root.innerHTML = `<p class="soft-note">${esc(inputs.emptyText || "No registered nodes")}</p>`;
      graphSlot = null;
      return;
    }
    root.innerHTML = `
      <header class="node-library-header">
        ${icon(nodeDefinitionIcon(definition))}
        <span><strong>${esc(definition.name)}</strong><small>${esc(definition.id)} · v${esc(definition.version)}</small></span>
        <em>${esc(inputs.contextLabel || definition.implementation?.kind || "node")}</em>
      </header>
      <p>${esc(definition.description)}</p>
      <div class="node-structure-canvas">
        ${definitionPortColumn("Inlets", definition.inlets, "inlet")}
        ...
        ${definitionPortColumn("Outlets", definition.outlets, "outlet")}
      </div>
      <div class="ui-node-definition-studio-graph" data-ui-node-definition-graph></div>
      <div class="node-library-parts">
        <h2>Editable parts</h2>
        <div>${(definition.parts || []).map((part) => `
          <article>${icon(definitionPartIcon(part.kind))}<strong>${esc(part.name || part.id)}</strong><small>${esc(part.kind)}${part.editable === false ? " · read only" : " · editable"}</small></article>
        `).join("") || '<p class="soft-note">Native node contract; no editable source parts</p>'}</div>
      </div>`;
    graphSlot = root.querySelector("[data-ui-node-definition-graph]");
  }

  function slot(name = "graph") {
    return name === "graph" ? graphSlot : null;
  }
...
function createNodeGraphEditorInstance({ host, inputs: initialInputs, document, capabilities, emit }) {
  let inputs = initialInputs || {};
  let root = null;

  function mount() {
    root = document.createElement("div");
    root.className = "ui-node-graph-editor";
    root.dataset.uiNodeOwned = "node-graph-editor";
    host.replaceChildren(root);
    update(inputs);
  }
  ...
  function update(nextInputs = {}) {
    inputs = nextInputs;
    const registry = capabilities?.nodeDefinitions;
    root.innerHTML = nodeGraphCanvasTemplate(inputs.definition, registry, inputs.options || {});
    bindNodeGraphCanvas(root, {
      registry,
      onGraphChange: (graph, reason) => emit("graph-change", { graph, reason }),
      onStatus: (message) => emit("status", { message: String(message || "") }),
      onMediaParameterRequest: ({ nodeId, parameterId, accept }) =>
        emit("media-request", { nodeId, parameterId, accept }),
      onPublicParameterToggle: (payload) => emit("public-parameter-toggle", payload),
      onPublicPortToggle: (payload) => emit("public-port-toggle", payload),
    });
  }
...
function nodeDefinitionIcon(definition = {}) {
...
function definitionPartIcon(kind) {
  return kind === "shader" ? "gradient"
    : kind === "graph" ? "account_tree"
      : kind === "asset" ? "deployed_code"
        : kind === "documentation" ? "description"
          : "code_blocks";
}

function definitionPortColumn(label, ports, role) {
  const entries = Object.values(ports || {});
  return `<div class="node-structure-ports is-${role}"><h2>${esc(label)}</h2>${entries.map((port) => {
    const type = typeof port.type === "string" ? port.type : port.type?.type || port.type?.id || "any";
    const range = port.expectedRange || port.allowedRange;
    const rangeText = Array.isArray(range) ? ` · ${range[0]}–${range[1]}` : "";
    return `<div><i></i><span><strong>${esc(port.label || port.id)}</strong><small>${esc(type)}${esc(rangeText)}</small></span></div>`;
