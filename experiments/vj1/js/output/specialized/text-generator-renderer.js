...
function layoutTextLines(context, lines, options) {
...
function measureStyledLine(context, line, size, family, spacingRatio, textStyle = {}) {
  let width = 0;
  let characters = 0;
...
function drawStyledLine(context, line, startX, baseline, size, family, spacingRatio, textStyle = {}) {
  let x = startX;
  for (const run of line.runs) {
    context.font = fontString(textStyle, size, family);
