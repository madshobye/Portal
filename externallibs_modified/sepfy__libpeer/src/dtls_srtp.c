...
static const unsigned char dtls_srtp_static_key[] =
    "-----BEGIN EC PRIVATE KEY-----\n"
    "MHcCAQEEIKH6LWl5qMbUSTVY8LxCVudo0XSLWkBBTAcq0ycDSeStoAoGCCqGSM49\n"
    "AwEHoUQDQgAEGYnZ2TLaKA9Wcx3MeM9/ewvvqEuhmg/pbqXAJOJuFafuhlCiRmFQ\n"
    "uTMkKxzxowkAkSNajeKnpu+lIoWASbeR4A==\n"
    "-----END EC PRIVATE KEY-----\n";

static const unsigned char dtls_srtp_static_cert[] =
    "-----BEGIN CERTIFICATE-----\n"
    "MIIBFzCBvgIJAJBPnH+gA9uWMAoGCCqGSM49BAMCMBQxEjAQBgNVBAMMCWR0bHNf\n"
    "c3J0cDAeFw0yNjA1MjEyMTI5NThaFw0zNjA1MTgyMTI5NThaMBQxEjAQBgNVBAMM\n"
    "CWR0bHNfc3J0cDBZMBMGByqGSM49AgEGCCqGSM49AwEHA0IABBmJ2dky2igPVnMd\n"
    "zHjPf3sL76hLoZoP6W6lwCTibhWn7oZQokZhULkzJCsc8aMJAJEjWo3ip6bvpSKF\n"
    "gEm3keAwCgYIKoZIzj0EAwIDSAAwRQIhAKBuU+bbXQ+LG83g1+STK6ofXk5oZVRe\n"
    "4F8GwPPuAtSYAiAzvIlfZ24QUcPN6k9WBVUn/Z6P1pvxc9TLsKum3E4vJA==\n"
    "-----END CERTIFICATE-----\n";
...
static int dtls_srtp_selfsign_cert(DtlsSrtp* dtls_srtp) {
  ...
  ret = mbedtls_x509_crt_parse(&dtls_srtp->cert, dtls_srtp_static_cert, sizeof(dtls_srtp_static_cert));
  if (ret < 0) {
    LOGE("mbedtls_x509_crt_parse static failed -0x%.4x", (unsigned int)-ret);
    return ret;
  }

  ret = mbedtls_pk_parse_key(&dtls_srtp->pkey, dtls_srtp_static_key, sizeof(dtls_srtp_static_key), NULL, 0, mbedtls_ctr_drbg_random, &dtls_srtp->ctr_drbg);
  if (ret < 0) {
    LOGE("mbedtls_pk_parse_key static failed -0x%.4x", (unsigned int)-ret);
    return ret;
  }

  LOGI("using static DTLS certificate");
  return 0;
...
#endif
  const size_t cert_buf_len = 8192;
  ...
  ret = mbedtls_x509write_crt_pem(&crt, cert_buf, cert_buf_len, mbedtls_ctr_drbg_random, &dtls_srtp->ctr_drbg);
  ...
  ret = mbedtls_x509_crt_parse(&dtls_srtp->cert, cert_buf, strlen((const char*)cert_buf) + 1);
  if (ret < 0) {
